...
<template>
  <button
    class="lang-switcher"
    :class="[`lang-switcher--${variant}`, { 'is-en': !isSpanish }]"
...
/* Admin backoffice: teal BI palette, matches currency-switcher + icon-btn */
.lang-switcher--admin {
  --switcher-h: 40px;
}

.lang-switcher--admin .lang-track {
...
.lang-switcher--admin.is-en .lang-track {
  background: var(--bi-paper, #ffffff);
  border-color: var(--bi-border, #e2e8f0);
  box-shadow: none;
}

.lang-switcher--admin .lang-thumb {
  top: 3px;
  left: 3px;
  height: calc(var(--switcher-h) - 6px);
  border-radius: 7px;
  background: linear-gradient(135deg, #0f766e 0%, #0d655e 100%);
  box-shadow: 0 1px 4px rgba(15, 118, 110, 0.28);
}

.lang-switcher--admin.is-en .lang-thumb {
  background: linear-gradient(135deg, #334155 0%, #475569 100%);
  box-shadow: 0 1px 4px rgba(51, 65, 85, 0.22);
}

.lang-switcher--admin:focus-visible .lang-track {
  outline: 2px solid var(--bi-teal, #0f766e);
  outline-offset: 2px;
  box-shadow: none;
}

.lang-switcher--admin:hover .lang-track {
  border-color: color-mix(in srgb, var(--bi-teal, #0f766e) 30%, var(--bi-border, #e2e8f0));
  background: var(--bi-bg, #eef2f6);
  box-shadow: none;
}

.lang-switcher--admin.is-en:hover .lang-track {
  border-color: color-mix(in srgb, var(--bi-teal, #0f766e) 30%, var(--bi-border, #e2e8f0));
  background: var(--bi-bg, #eef2f6);
  box-shadow: none;
}

.lang-switcher--admin .lang-text {
  color: var(--bi-muted, #64748b);
  letter-spacing: 0.06em;
}

.lang-switcher--admin .lang-label.is-active .lang-text {
  color: #ffffff;
  text-shadow: none;
}
</style>
